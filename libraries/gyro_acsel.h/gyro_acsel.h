...
void forward_t( long microsec)
{
  ...
  do {
    if (Ang_ < (-3.0 + Ang_2)) {
      forward_left(); time_gyro(0.5); microsec += 500;
    }
    else if (Ang_ > (3.0 + Ang_2)) {
      forward_right(); time_gyro(0.5); microsec += 500;
    }
    else {
      forward(); time_gyro(0.5); _stop(); time_gyro(0.1);
    }
...
void backward_t(unsigned long microsec)
{
  ...
  do {
    if (Ang_ < (-3.0 + Ang_2)) {
      backward_right(); time_gyro(0.5); microsec += 500;
    }
    else if (Ang_ > (3.0 + Ang_2)) {
      backward_left(); time_gyro(0.5); microsec += 500;
    }
    else {
      backward(); time_gyro(0.5); _stop(); time_gyro(0.1);
    }
  } while (microsec > micros());
  _stop();
  time_gyro(0.5);
...
void Angle(float ang)
{
  ...
  float angX = ang;
  float stop_d = 1;
  while ((ang > 2.5) || (ang < -2.5))
  {
    if (ang < -3) ang = -3.0;
    else if (ang > 3) ang = 3.0;
    if (ang < 0)
    {
      do {
        right(); time_gyro(1); _stop(); time_gyro(stop_d);
      } while (Ang_ > (ang + Ang_2));
    }
    else
    {
      do {
        left(); time_gyro(1); _stop(); time_gyro(stop_d);
      } while (Ang_ < (ang + Ang_2));
    }
    _stop();
    time_gyro(10);
